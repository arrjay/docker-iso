...
isoinfo -R -x '/boot/grub/grub.cfg' -i "${cd}" | sed -e 's/console=ttyS1,115200/console=tty0/' > "${newfiles}/boot/grub/grub.cfg"
mkdir -p "${newfiles}/isolinux"
isoinfo -R -x '/isolinux/syslinux.cfg' -i "${cd}" | sed -e 's/serial 1 115200//' -e 's/console=ttyS1,115200/console=tty0/' > "${newfiles}/isolinux/syslinux.cfg"
