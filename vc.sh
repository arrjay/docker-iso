...
isoinfo -x '/BOOT/GRUB/GRUB.CFG;1' -i "${cd}" | sed -e 's/console=ttyS1,115200/console=tty0/' > "${newfiles}/boot/grub/grub.cfg"
mkdir -p "${newfiles}/isolinux"
isoinfo -x '/ISOLINUX/SYSLINUX.CFG;1' -i "${cd}" | sed -e 's/serial 1 115200//' -e 's/console=ttyS1,115200/console=tty0/' > "${newfiles}/isolinux/syslinux.cfg"
